...
本项目没有试图重新发明聚类或空间统计方法。我的判断是，对于课程项目和真实探索性分析而言，
更有价值的问题是：能不能把成熟方法组织成一个不黑箱、能复核、失败后知道如何处理的系统。
因此，SpatialScope 的创新主要体现在工作流组织和解释约束上。它把 notebook 中经常散落在代码、
图表和口头解释里的信息，整理成一个浏览器中可连续体验的研究过程。

关键设计取舍包括：

- 标准空间转录组流程被显式 Agent 化：数据检查、计划审阅、执行、校验、修复、解释和报告都有状态记录。
- LLM 被限制在 evidence pack 与 schema validation 内，不接触完整矩阵，也不直接给出细胞类型或机制结论。
