...
\begin{titlepage}
  ...
  {\color{white}\Large 面向空间转录组的证据驱动分析智能体}\\[1.3cm]

  \begin{summarybox}{项目一句话}
    SpatialScope Agent 将自然语言研究问题转化为可审阅的 LangGraph 分析流程，
    以 Scanpy/Squidpy 风格的确定性工具完成空间转录组分析，并把图表、统计摘要、
    LLM 解释、证据编号和可复现文件组织成一个可交互的科研工作台。
  \end{summarybox}
  ...
  \begin{center}
    ...
    {\small\color{scopeSlate}SpatialScope 主界面：中文友好、LLM 状态、Demo 入口与证据预览。}
  \end{center}
\end{titlepage}
...
我对该项目的设计目标不是写一个固定脚本，而是构建一个“证据工作台”：Agent 必须先检查
数据再规划，先生成工具证据再解释，并在最终报告中保留 evidence IDs、参数、图表、trace、
caveats 与可复现元数据。项目已经上传 GitHub，并完成 Streamlit Cloud 在线部署。在线
demo 经浏览器端测试可以完成从选择数据、生成方案、批准运行、查看 Explore 到打开 Report
的完整流程；本地测试通过 99 项单元测试，并在真实 GSE278603 E7.5 mouse embryo Stereo-seq
样本上完成 quick analysis smoke run。
...
数据示例；细胞通讯和高级模型作为后续路线图保留，没有在 v1 中强行堆叠。
...
\section{设计原则：把 Agent 做成证据工作台}

空间转录组分析的难点不只是“会不会调用 Scanpy”。真正容易出问题的是：用户以自然语言提出
问题，而数据对象、表达层、坐标字段、聚类结果和图表解释之间存在复杂依赖。如果 Agent 没有
状态、计划、工具契约和证据边界，它很容易退化为一个固定 notebook 或者一个会写漂亮话的聊天框。
因此本项目采用以下设计原则。
...
\begin{decisionbox}{原则三：每条结论必须能回到证据}
  ...
  和 \breakcode{gene:Pou5f1:summary}。这使用户可以从解释回到图表、表格和工具输出，而不是相信一段
  不可追踪的文字。
\end{decisionbox}

\begin{decisionbox}{原则四：主界面服务研究流程，底层审计进入 Advanced}
...
图 \ref{fig:architecture} 展示了 SpatialScope 的核心 Agent 流程。它不是单次函数调用，而是一个
可中断、可恢复、可校验、可修复的状态机。用户问题进入系统后，Agent 先检查数据，再将自然语言
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  node distance=1.35cm and 1.25cm,
  box/.style={rounded corners=2mm, draw=scopeTeal, fill=scopeMist, thick, align=center, minimum width=2.55cm, minimum height=0.95cm, font=\small},
  side/.style={rounded corners=2mm, draw=scopePlum, fill=scopePlum!8, thick, align=center, minimum width=2.75cm, minimum height=0.95cm, font=\small},
  warn/.style={rounded corners=2mm, draw=scopeGold, fill=scopeGold!12, thick, align=center, minimum width=2.75cm, minimum height=0.95cm, font=\small},
  arr/.style={->, thick, color=scopeTeal},
  loop/.style={->, thick, color=scopeGold}
]
\node[box] (user) {自然语言\\研究问题};
\node[box, right=of user] (inspect) {inspect\\dataset};
\node[box, right=of inspect] (parse) {parse\\request};
\node[box, right=of parse] (plan) {plan\\analysis};
\node[warn, below=of plan] (review) {human\\plan review};
\node[box, left=of review] (exec) {execute\\tool};
\node[box, left=of exec] (validate) {validate\\result};
\node[warn, below=of validate] (repair) {repair /\\clarify};
\node[box, left=of validate] (interpret) {interpret\\evidence};
\node[box, left=of interpret] (report) {report /\\Explore};
\node[side, above=of parse] (llm) {GLM 5.1 /\\LLM sidecar};
\node[side, above=of exec] (tools) {Scanpy-style\\tool registry};
\node[side, below=of exec] (state) {LangGraph state\\trace + metadata};

\draw[arr] (user) -- (inspect);
\draw[arr] (inspect) -- (parse);
\draw[arr] (parse) -- (plan);
\draw[arr] (plan) -- (review);
\draw[arr] (review) -- (exec);
\draw[arr] (exec) -- (validate);
\draw[arr] (validate) -- (interpret);
\draw[arr] (interpret) -- (report);
\draw[loop] (validate) -- (repair);
\draw[loop] (repair) -- (exec);
\draw[arr] (llm) -- (parse);
\draw[arr] (llm) -- (plan);
\draw[arr] (llm) to[bend left=10] (interpret);
\draw[arr] (tools) -- (exec);
\draw[arr] (state) -- (exec);
\draw[arr] (state) -- (validate);
\end{tikzpicture}
}
\caption{SpatialScope Agent 的 LangGraph 架构。LLM 是理解和解释侧车，确定性分析由工具层完成；状态、trace、metadata 和 repair 贯穿全流程。}
...
项目使用 OpenAI-compatible 接口接入 GLM 5.1。LLM 不是“万能分析器”，而是被放在清晰边界内：
...
不仅成本高，也会让结果不可复现。SpatialScope 的做法是：工具先产生可追踪证据，LLM 只能围绕
证据组织语言。
...
Run 页面以 live events 展示 LangGraph 节点进度。这个设计是为了让用户知道 Agent 正在做什么，
而不是面对一个不可解释的长时间 spinner。
...
实现时我特别处理了表达来源安全问题。对于 count-like \code{X}，系统会生成归一化和 log1p 的
...
在提交前，我对公开 Streamlit app 做了浏览器端完整验收：
...
真实数据验证给项目带来两个加分点。第一，它证明系统不只是为 synthetic demo 写死流程，能够识别
真实数据中的 \code{X_spatial} 坐标并标准化为 \code{spatial}。第二，它暴露了真实数据更复杂的表达
来源问题：矩阵被判断为 log-normalized，系统需要优先选择 raw 或安全 interpretation layer，而不是
盲目把当前 \code{X} 当作 counts 解释。
...
相对于一个固定 Scanpy notebook，本项目的创新点主要体现在以下几个方面：

\begin{enumerate}
  \item \textbf{Agent 化的空间组学流程。} 使用 LangGraph 显式建模节点、状态、条件边和 repair loop，
  让分析过程可解释、可中断、可恢复。
  \item \textbf{证据约束的 LLM。} LLM 只围绕 evidence pack 解释，不接触完整矩阵，并显示 evidence IDs。
  ...
  \item \textbf{中文友好的科研产品体验。} 主界面围绕 Project、Run、Explore、Report 组织，保留必要英文术语，
  同时加入 logo、signature tags、Acknowledgements 和 Scopelet Copilot 角色感。
  \item \textbf{真实数据可验证。} 提供 GSE278603 下载脚本和真实 E7.5 样本 quick run，而不是只依赖 toy data。
\end{enumerate}
...
未来可以继续扩展：引入更强的空间可变基因统计、多 run 对比、marker ontology 映射、细胞类型注释建议、
PDF/Word 自动报告导出，以及面向真实大型数据的远程计算后端。

\section{总结}

SpatialScope Agent 完成了从作业要求到高质量项目形态的转化。它不仅能完成空间转录组基础分析，
还把 Agent 的核心思想 - 状态、工具、计划、循环、修复、记忆和解释 - 变成了用户可以在浏览器中
实际体验的流程。项目的核心价值不在于某一个单独算法，而在于把自然语言、空间组学工具、LLM 解释、
可视化和可复现性组织成一个整体。

从评分角度看，系统完整性、空间转录组分析功能、创新性、报告和展示都已有明确支撑：GitHub 可运行，
在线部署可访问，测试和真实数据 run 已通过，报告和 demo script 已准备。后续课堂展示中，最应该强调的
不是“我调用了哪些包”，而是“我如何让 Agent 先理解数据、再制定计划、再生成证据、最后谨慎解释”。
