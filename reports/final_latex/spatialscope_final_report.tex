...
本项目的创新性并不在于提出新的聚类算法或替代已有空间统计方法，而在于将成熟的空间转录组分析流程
重构为一个可审阅、可修复、证据约束、可部署的科研 Agent。与固定 Scanpy notebook 相比，它把
“一次性脚本执行”提升为“围绕数据、计划、工具、证据和解释持续流动的研究工作台”。这一点也是
答辩时最值得强调的主线：SpatialScope 不是简单的可视化界面，而是一个把计算流程、LLM 推理和科学
审慎性连接起来的系统。

\begin{summarybox}{创新点概括}
  SpatialScope 的核心贡献可以概括为三句话：第一，标准空间转录组流程被显式 Agent 化；
  第二，LLM 被限制在 evidence pack 和 schema validation 内；第三，用户可以在浏览器中看到
  数据检查、计划审阅、实时执行、联动探索和带 caveat 的 Research Brief。
\end{summarybox}
