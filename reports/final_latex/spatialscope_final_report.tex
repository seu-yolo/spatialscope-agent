...
\pagestyle{fancy}
\fancyhf{}
\lhead{\raisebox{-0.18\height}{\includegraphics[height=0.42cm]{assets/logo.png}}\hspace{0.35em}\small SpatialScope Agent}
...
\newcommand{\screenshot}[1]{%
  ...
  \fcolorbox{scopeLine}{white}{\includegraphics[width=\dimexpr\linewidth-2\fboxrule\relax]{#1}}%
  \endgroup
}
...
在实现过程中，我逐渐意识到，空间转录组 Agent 最难的部分不是把 QC、UMAP 或 marker ranking
串起来，而是让系统在每一步都知道自己“凭什么可以继续”。因此，我把重点放在数据先验检查、
表达层安全、证据编号和解释边界上，而不是追求让 LLM 看起来更自由。
...
\begin{tikzpicture}[
  ...
  main/.style={rounded corners=2mm, draw=scopeTeal!92!black, fill=scopeMist, line width=0.75pt,
    align=center, text width=2.12cm, minimum width=2.28cm, minimum height=1.02cm, font=\footnotesize},
  ...
  support/.style={rounded corners=2mm, draw=scopeBlue!80!black, fill=scopeBlue!6, line width=0.7pt,
    align=center, minimum height=0.84cm, font=\footnotesize},
...
\node[main] (parse) at (2.85,0) {任务解析\\ResearchBrief};
\node[review] (plan) at (5.70,0) {方案审阅\\人工批准};
\node[main] (execute) at (8.55,0) {工具执行\\QC / UMAP};
\node[review] (validate) at (11.40,0) {校验修复\\安全检查};
\node[main] (report) at (14.25,0) {证据报告\\Explore / Report};

\node[support, minimum width=6.45cm] (llm) at (4.28,1.62) {LLM 支持模块（GLM 5.1）\\理解问题、补全计划、生成证据约束解释};
\node[support, minimum width=4.95cm] (tools) at (8.55,-1.85) {确定性工具层\\Scanpy / AnnData / 图表生成};
\node[statebox, minimum width=5.30cm] (state) at (12.82,1.62) {状态与证据层\\trace、metadata、parameters、evidence IDs};

\draw[arrow] (inspect.east) -- (parse.west);
\draw[arrow] (parse.east) -- (plan.west);
\draw[arrow] (plan.east) -- (execute.west);
\draw[arrow] (execute.east) -- (validate.west);
\draw[arrow] (validate.east) -- (report.west);

\draw[looparrow] (validate.south) -- ++(0,-0.82) node[midway,right,font=\scriptsize,color=scopeGold!85!black]{失败或不安全} -| (execute.south);

\draw[softarrow] (llm.south west) -- ([yshift=0.04cm]parse.north);
\draw[softarrow] (llm.south east) -- ([yshift=0.04cm]plan.north);
\draw[softarrow] (tools.north) -- ([yshift=-0.04cm]execute.south);
\draw[softarrow] (state.south west) -- ([yshift=0.04cm]validate.north);
\draw[softarrow] (state.south east) -- ([yshift=0.04cm]report.north);
\end{tikzpicture}
}
...
\begin{subfigure}[t]{0.49\textwidth}
  \screenshot{assets/app_landing.png}
  \caption{项目入口：自然语言问题、demo 数据、LLM 状态和视觉签名。}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
  \screenshot{assets/app_dataset_understood.png}
...
\centering
\begin{subfigure}[t]{0.49\textwidth}
  \screenshot{assets/app_plan_review.png}
  \caption{Plan Review：7 步标准分析方案和可调参数。}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
  \screenshot{assets/app_run_live.png}
...
\centering
\begin{subfigure}[t]{0.49\textwidth}
  \screenshot{assets/app_explore.png}
  \caption{Explore：Spatial + UMAP 联动，Copilot 绑定当前 evidence。}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
  \screenshot{assets/app_report.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/demo_gene_panel.png}
...
本项目使用 NCBI GEO 公开数据集 GSE278603 作为真实数据 smoke test。该 series 的研究背景与
2025 年 \textit{Cell} 论文 ``Digital reconstruction of full embryos during early mouse organogenesis''
一致，均围绕早期小鼠胚胎的 Stereo-seq 空间转录组图谱。本项目提供
\breakcode{scripts/download_real_demo.sh} 下载脚本，从 GEO 官方补充文件中提取
\breakcode{GSM9046244_Embryo_E7.5_stereo_rep2.h5ad}。该文件约 31 MB，适合课程项目 smoke test。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/real_gene_panel.png}
...
\begin{summarybox}{设计取舍}
  我没有让 LLM 直接读取完整表达矩阵，也没有让它直接给出细胞类型或机制结论。这个选择看起来
  更保守，但更符合我对科研工具的理解：空间转录组解释中最危险的不是结论不够丰富，而是证据
  边界不清楚。SpatialScope 因此把 LLM 放在问题理解、计划组织和证据解释的位置，把计算交给
  可复现工具，把每条解释绑定到 evidence IDs。
\end{summarybox}
...
SpatialScope Agent 完成了一个可运行、可部署、可复核的空间转录组分析系统。系统不仅覆盖
\code{.h5ad} 读取、质量控制、预处理、降维聚类、空间可视化和 marker gene 分析等基础功能，还将
Agent 的状态、工具、计划、循环、修复和解释机制整合到浏览器端流程中。

回看这个项目，我认为最重要的收获是：Agent 的价值不是让模型说得更多，而是让分析过程更容易被追问。
当用户看到数据检查、计划审阅、工具 trace、evidence IDs 和 caveats 时，他不仅得到一组图，也能知道
这些图是在什么假设下产生的、哪些解释可以相信、哪些地方仍然需要人工判断。这个边界感是 SpatialScope
想保留下来的核心。
...
\begin{enumerate}
  ...
  \textit{Genome Biology} 19(1), 15 (2018). DOI: \url{https://doi.org/10.1186/s13059-017-1382-0}
  \item Palla, G. et al. Squidpy: a scalable framework for spatial omics analysis. \textit{Nature Methods} 19(2), 171-178 (2022).
  DOI: \url{https://doi.org/10.1038/s41592-021-01358-2}
  \item Virshup, I. et al. anndata: Access and store annotated data matrices. \textit{Journal of Open Source Software}
  9(101), 4371 (2024). DOI: \url{https://doi.org/10.21105/joss.04371}
  \item Righelli, D. et al. SpatialExperiment: infrastructure for spatially-resolved transcriptomics data in R using Bioconductor.
  \textit{Bioinformatics} 38(11), 3128-3131 (2022). DOI: \url{https://doi.org/10.1093/bioinformatics/btac299}
  \item Crowell, H. L. et al. Orchestrating spatial transcriptomics analysis with Bioconductor. \textit{bioRxiv} (2025).
  DOI: \url{https://doi.org/10.1101/2025.11.20.688607}; online book: \url{https://www.bioconductor.org/books/OSTA/}
  ...
  \item LangGraph documentation. \url{https://docs.langchain.com/oss/python/langgraph/overview}
  \item Streamlit documentation. \url{https://docs.streamlit.io/}
  \item NCBI GEO Series GSE278603: Digital reconstruction of spatiotemporal atlas unveils the cellular origins and signaling networks underlying early mouse organogenesis.
  \url{https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE278603}
  \item Xie, P., Shen, J., Yang, Y., et al. Digital reconstruction of full embryos during early mouse organogenesis.
  \textit{Cell} 188(17), 4754-4772.e18 (2025).
