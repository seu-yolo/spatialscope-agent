...
TOOL_REGISTRY: dict[str, ToolSpec] = {
    ...
    "run_preprocess": ToolSpec(
        ...
        description="Create explicit expression layers, normalize/log-transform when appropriate, select highly variable genes, and scale model input.",
        contract=ToolContract(
            name="run_preprocess",
            required_fields=["adata"],
            ...
            postconditions=[
                "adata.layers['spatialscope_interpretation'] is available for plots and marker ranking.",
                "adata.layers['spatialscope_model_scaled'] is available for model-facing dimensionality reduction.",
                "adata.uns['spatialscope_expression_lineage'] records matrix-state assumptions.",
                "adata.var['highly_variable'] is available when possible.",
            ],
            common_failures=["Dataset has too few genes after QC.", "Scanpy is not installed.", "Input matrix state is unknown."],
            repair_strategy=[
                "Reduce QC stringency.",
                "Lower n_top_genes.",
                "Use explicit input layers if counts/log-normalized data are known.",
                "Install the core conda environment.",
            ],
        ),
    ),
    ...
    "plot_spatial": ToolSpec(
        ...
        contract=ToolContract(
            ...
            optional_fields=["color", "expression_layer", "clip_percentiles"],
            preconditions=["Spatial coordinates exist in adata.obsm['spatial']."],
            postconditions=["A spatial PNG is written to the run figure directory.", "Gene plots record the expression layer and clipping used."],
    ...
    "plot_gene_panel": ToolSpec(
        ...
        contract=ToolContract(
            ...
            optional_fields=["expression_layer", "clip_percentiles"],
            preconditions=["Spatial coordinates exist.", "At least one requested gene can be matched."],
            postconditions=["A gene panel PNG is written.", "Resolved gene names, expression layer, and clipping are recorded in observations."],
    ...
    "rank_markers": ToolSpec(
        ...
        contract=ToolContract(
            ...
            optional_fields=["groupby", "top_n", "expression_layer"],
            preconditions=["A clustering column such as leiden exists."],
            postconditions=["Marker tables and a top-marker heatmap are written.", "The marker-ranking expression layer is recorded."],
    ...
    "suggest_cluster_annotations": ToolSpec(
        ...
        description="Suggest cautious candidate cluster labels only when a compatible marker reference is explicitly selected.",
        contract=ToolContract(
            name="suggest_cluster_annotations",
            required_fields=["adata.obs[groupby]", "adata.uns['rank_genes_groups']"],
            optional_fields=["groupby", "top_n", "reference"],
            preconditions=["Marker ranking has completed.", "A clustering column such as leiden exists.", "A compatible reference is explicitly selected."],
            postconditions=["Candidate annotation table and marker-evidence figure are written."],
            common_failures=["Marker rankings are missing.", "Markers do not overlap the compact lexicon.", "No reference was explicitly selected."],
            repair_strategy=[
                "Run rank_markers first.",
                "Set reference='generic_marker_lexicon' only for exploratory marker-overlap suggestions.",
